...
export function buildManualProposalInput(input: ManualFormInput): ManualProposalInput {
  const title = `Manual ${input.operation.replace('_', ' ')}`;
  ...
  if (input.operation === 'remove_node') {
    return {
      baseVersionId: input.baseVersionId,
      title,
      op: { op: 'remove_node', id: requiredValue(input.targetNodeId, 'node id') },
    };
  }
  ...
  return {
    baseVersionId: input.baseVersionId,
    title,
    op: { op: 'remove_edge', from: edge.from, to: edge.to, edge_type: edge.edge_type },
  };
}
