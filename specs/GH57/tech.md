...
| Run 执行路径 | `crates/run/src/lib.rs:163-420`、`crates/run/src/cost_gate.rs:24/:71`、server sweep support | `RunService` 泛型持单一 provider;`execute_manual_run`(lib.rs:208-211)在内部调 `compile_plan`;`request_agent_run` 与 `request_sweep_plan(SweepPlan)` 在 cost_gate;sweep server support 构造 `SweepPlan`;invoke 用 `step.provider` 构造 `ProviderRequest`(lib.rs:408-411) | manual/agent/sweep 请求都需携带已解析的 provider id 并透传给 `compile_plan`;registry 实现 `Provider` 后 `RunService<ProviderRegistry>` 按 `req.provider` 分发 |
| 产物持久化 | `crates/gateway/src/lib.rs`(`ProviderResult`/`ArtifactPayload`)、`crates/run/src/lib.rs:575`(`persist_artifact`)、`crates/server/src/artifact_routes.rs`、`crates/server/src/workbench_payload.rs` | `ProviderResult.outputs` 当前携带 `ArtifactPayload.storage_uri`,但 provider 没有表达 inline bytes 的字段;`persist_artifact` 只把 storage_uri 字符串写库;preview/download 路由只回元数据 + `safe_download_uri`,没有任何组件落盘或回源真实字节 | Atlas 返回的真实图片字节需要 gateway 输出契约、RunService 落盘、safe URI/content 端点和 preview schema 同步变更(见设计方案 §8) |
...
- **节点定义声明"能力"而非绑定 provider**:`crates/registry/src/lib.rs` 的可执行节点改为 provider 中立的 `image.generate`(label "Generate Image")与 `video.text_to_video`(label "Text To Video"),描述文案不含 Mock;定义保留 `capability`(`image_generate`/`text_to_video`),`provider` 绑定移除(置 `None`)。`llm.prompt_writer` 同样移除 provider 绑定,但只有在 selected provider 声明支持 `prompt_writer` 时才能运行;Atlas 必须把 `prompt_writer` capability 映射到其 `chat_completion` 实现,否则使用现有 prompt-writer 节点的 Atlas 文生图工作流会在到达 image 之前失败。catalog 的 `estimated_cost` key 从 `{provider}.{capability}` 改为按 capability 生成(计费/配额是非目标,估价目录不按 provider 分叉)。
...
- **RunService 入口透传 workspace 选择**:`ManualRunRequest`、`AgentRunRequest` 与 `SweepPlan` 增加必填字段 `provider: String`(已解析的生效 provider id);`execute_manual_run` / `request_agent_run` / `request_sweep_plan` 原样传给 `compile_plan`,自身不做解析。解析(workspace 选择 → NULL 时服务器默认)由 server 入口在入队或创建 pending sweep 前完成(见 §7)。
- **mock 降级为一个普通 provider**:`image.mock.generate`/`video.mock.text_to_video` classType **删除,不留兼容层**;mock 作为 registry 中的普通 provider 实现同样的 `prompt_writer`/`image_generate`/`text_to_video` capability。所有现有引用这些 classType 的图与测试同步更新为新 classType(内置模板图、`crates/graph`/`crates/run`/`crates/agent`/`crates/server` 测试、`web/src/app.test.tsx`),列入任务(SP57-T9 及各 lane 所属文件)。
...
- `queue_workspace_run`、agent run 入口、seed/sweep 入口在入队/建 pending sweep 时解析生效 provider id(workspace 选择 → NULL 时服务器默认),分别填入 `ManualRunRequest.provider`、`AgentRunRequest.provider`、`SweepPlan.provider` 字段;`compile_plan` 将其写进每个 step,持久化到 run steps 的 `provider` 列;run/sweep 执行期间不再重新解析——运行中切换选择不影响 in-flight run。
- seed sweep 支持必须在创建 pending sweep runs 前检查生效 provider 可用性,并把 provider 固定在 `SweepPlan`/每个成员 run step 上;不能让 sweep support 内部构造 `AgentRunRequest` 时绕过 workspace provider 选择。
...
- **gateway 输出类型**:扩展现有 `crates/gateway/src/lib.rs::ArtifactPayload`,不要引用不存在的 `ProviderResponse` 类型。新增 provider-side content 字段(例如 `content: ArtifactContent`,其中 `ArtifactContent::InlineBytes { bytes, ext_hint }` 表示 provider 已返回字节,`ArtifactContent::RemoteUrl { url }` 表示需要 RunService 拉取一次);provider 输出不再负责填写最终 `storage_uri`。`ArtifactRef.storage_uri` 仍仅用于 run step 输入引用已持久化 artifact,不用于 provider 新输出。
- **落盘**:`RunService` 把 `ArtifactPayload.content` 表达的字节(或先下载其 HTTPS URL 内容)写入 `data_dir` 下 `artifacts/{run_id}/{step_index}.{ext}`(ext 由 mime 或 `ext_hint` 推导);`RunService` 构造时由 server 注入 artifacts 根目录(`data_dir/artifacts`)。mock 产物迁移到同一落盘契约,消除双轨。
...
- **preview/download 走它**:`crates/server/src/workbench_payload.rs::safe_download_uri` 与 artifact preview/download 改为指向该 content 端点;现有"不暴露原始 storage_uri"的测试语义保留。
- **前端真实媒体预览**:workspace output preview schema 与 `ArtifactStage` 增加 `image`/`video` preview kind,内容为安全 content 端点 URL 与 mime/尺寸元数据;图片用 `<img>`、视频用 `<video controls>` 渲染。`html` preview 仍保持 sandbox,`text` preview 仍按文本渲染;真实 Atlas image/video 不得退化为 metadata text。

### 9. Atlas provider(第一个真实 provider)

- 从 salvage 捞回:`git fetch origin salvage/local-workbench-atlas-20260702 && git show FETCH_HEAD:crates/server/src/provider.rs`(commit `4f3e5ea`,分支已推送到 origin)。`from_env()` 读 `ATLAS_API_KEY`/`ATLAS_API_BASE`(默认 `https://api.atlascloud.ai/v1`)、bearer 鉴权、`image_generate` 同步生成、`text_to_video` 提交 + 轮询、`chat_completion`。Atlas catalog 必须声明并实现 `prompt_writer` capability,实现可复用 `chat_completion` 返回 text artifact,保证现有 prompt_writer 节点在 atlas workflow 中可执行。本 issue 验收只卡 `image_generate` 端到端;`prompt_writer`、`chat_completion`、`text_to_video` 以单测覆盖,不作为手工 e2e 阻塞。
- `estimate` 契约:Atlas 对 `prompt_writer`/`image_generate`/`text_to_video` 必须返回安全的 `CostEstimate { estimated: true, ... }`。若 Atlas 没有稳定估价 API,使用本地静态估算,不得因估价 unsupported 或无估价 API 让 agent run 在 `estimating` 阶段失败。
...
- `web/src/components/artifact-stage.tsx` 与 `web/src/types.ts` 同步支持 §8 的 image/video preview kind,并补 `app.test.tsx` 覆盖真实图片 preview 使用 content URL 而非 metadata text。
- 不做设置面板、不做凭据输入。
...
| P6 Atlas 文生图 e2e,产物字节可预览/下载 | `atlas.rs::invoke(image_generate)`、gateway `ArtifactPayload.content`、`RunService` 落盘、`GET /api/artifacts/{id}/content`、前端 image/video preview | gateway/run 单测:ArtifactPayload inline bytes/remote URL 均能落盘且 `storage_uri` 为相对路径;server 测试:content 端点 mime/404/路径穿越拒绝、`safe_download_uri` 指向 content 端点;web 测试:图片 preview 渲染为 `<img>`;手动验证:设 `ATLAS_API_KEY` 跑文生图 workflow,产物可预览/下载 |
| P7 上游 401 明确失败且脱敏 | `atlas.rs` 错误映射、`ProviderError::RequestFailed` | gateway 单测(mock http/固定 401 响应体)断言消息无 bearer/key |
| P8 运行中切换不影响 in-flight run | 入队/sweep 建档解析 → run 请求/SweepPlan `provider` 字段 → `compile_plan` 参数固定到 run steps | graph 单测:`compile_plan` 把参数 id 写进每个 step;server run/sweep 测试:入队或创建 sweep 后改选择,断言 run/sweep steps 仍用原 provider |
| P9 mock 与真实并存,节点 provider 中立 | registry 装配、`crates/registry` 节点定义、Atlas prompt_writer/estimate、前端 gate | gateway snapshot 测试含双条目;registry 单测断言节点无 provider 绑定且 label 不含 Mock;Atlas 单测断言 `prompt_writer` 经 `chat_completion` 映射、`estimate` 对三种 capability 不失败;web `app.test.tsx` gate 用例 |
...
- 持久化:`workspaces.runtime_provider_id`(仅 provider id 字符串,绝不存凭据);`providers` 表存各 provider 健康状态快照;provider 输出通过 `ArtifactPayload.content` 表达 bytes/URL,RunService 将产物字节落盘 `data_dir/artifacts/{run_id}/{step_index}.{ext}`,`artifacts.storage_uri` 存该相对路径。
- 运行链路:入队或 sweep 建档时解析生效 provider(workspace 选择 → 服务器默认)→ run 请求/`SweepPlan.provider` 字段 → `compile_plan(graph, version_id, provider_id)` 写进每个 `ExecutionStep.provider` → 持久化 run steps → `ProviderRequest.provider` 分发到 registry。
- 输出:`GET /api/registry/catalog` 保持纯节点 catalog;workspace state `providers`(含 `selectedProvider` 与必要时的合成 unavailable 条目)承载 provider 目录;`GET /api/artifacts/{artifact_id}/content` 按 mime 回源产物字节,preview/download 指向它;workspace output preview 对 image/video 返回 content URL 并由前端媒体控件渲染;run events 与现有链路一致。
...
- [ ] Unit tests: gateway — registry 分发(命中/未注册拒绝)、多 provider snapshot 聚合、ArtifactPayload content 类型序列化/构造、Atlas `from_env`(有/无 key)、`prompt_writer` 映射到 `chat_completion`、`estimate` 对 `prompt_writer`/`image_generate`/`text_to_video` 返回安全静态估算、401 错误映射与消息脱敏、unavailable 原因文案通过 `is_safe_provider_message`。
...
- [ ] Unit tests: run — provider 返回 inline bytes 或 remote URL 均落盘到 `artifacts/{run_id}/{step_index}.{ext}`、`storage_uri` 为相对路径且非 http(s);run 请求与 `SweepPlan.provider` 原样透传到 steps。
- [ ] Unit tests: store — migration 后 `runtime_provider_id` 读写、NULL 回退语义。
- [ ] Integration tests: server — `PUT /api/workspaces/{id}/provider`(合法/未注册/空 id)、选择持久化跨 Store 重开、unavailable provider 入队/创建 sweep 前被拒(无产物)、queue/sweep 后切换选择不影响 in-flight run、workspace state 含 `selectedProvider`、持久化 id 未注册时合成 unavailable 条目、`GET /api/artifacts/{id}/content` 的 mime/404/路径穿越用例、`safe_download_uri` 指向 content 端点、`GET /api/registry/catalog` 响应 shape 不含 provider 字段。
- [ ] Integration tests: web — TopBar 选择器渲染双 provider 状态(含合成 unavailable 条目)、选择调用 API、gate 按 `selectedProvider` 判定、ArtifactStage 对 image/video preview 渲染媒体控件并使用 content URL(`app.test.tsx`,classType 同步更新为 `image.generate`/`video.text_to_video`)。
