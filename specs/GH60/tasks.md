...
- [ ] `SP60-T3` Chat skill 澄清指引 — Owner: single-agent。改 `crates/agent/src/lib.rs` `selected_skill(AgentSkill::Chat)`:追加"用户意图不明时主动澄清(创建/修改/运行/调试 workflow)"一行指引。Done when: Chat skill 文本含澄清指引且编译通过。Verify: `cargo check -p helixflow-agent`
