...
7. 持久化(保证成功版本永远指向已写好的图文件,且 409 路径最终零残留):先把新图写入临时文件(如 `ops-{uuid_v7}.json.tmp`)并计算 hash,再原子 rename 到最终路径 `workspaces/{workspace_id}/graphs/ops-{uuid_v7}.json`;只有最终文件已存在且可读后,才进入 store 的版本 commit gate。commit gate 必须在同一事务/锁内同时检查 `(workspace.cur_version_id == baseVersionId)` 与"不存在 pending agent proposal",然后写 versions 表并推进 `workspace.cur_version_id`。如果 commit gate 返回版本冲突或 pending proposal 冲突,ops 路由必须删除最终文件与临时文件后返回 409;如果 SQLite/IO/权限/ENOSPC 等 operational failure 发生,同样清理 final/tmp 文件,但响应必须走 server error,不能把服务端写入故障伪装成客户端 409;不能留下 orphan graph 文件或 `.tmp`。
