...
- [ ] `SP59-T1` Owner: backend-server. 新增 `crates/server/src/ops_routes.rs` 与 `POST /api/workspaces/{workspace_id}/versions/ops`:批量 `ManualOpRequest`(含 move_node)转 `ProposalOp`、原子校验(全成全败,逐 op 应用并跟踪 `opIndex`)、直接成版 source=manual;body 解析用自定义 rejection 映射(`WithRejection<Json<T>, ApiError>` 或手动 `from_request`)把 `JsonRejection` 转为结构化 400;扩展 `api_error.rs` 可选 `details` 输出 `opIndex`;图文件先 temp 写入并原子 rename 到最终路径,commit gate 在同一事务/锁内同时检查 base version 与 pending proposal,base-version/pending-proposal 冲突清理 final/tmp 文件并映射 409,SQLite/IO/权限/ENOSPC 等 DB 写入失败同样清理 final/tmp 文件但返回 server error,成功版本不得指向未写好的图文件。Done when: tech.md 设计方案第 1/2 节全部行为落地且 P1-P5、P7 对应单测与 HTTP 层未知字段集成测试通过。Verify: `cargo test -p helixflow-server`
...
- backend-server lane(SP59-T1 → SP59-T2):独占 `crates/server/src/ops_routes.rs`(新建)、`crates/server/src/main.rs`、`crates/server/src/api_error.rs`、`crates/server/src/manual_proposal_routes.rs`(删)、`crates/server/src/manual_proposal_routes_tests.rs`(删),并允许在 `crates/store/src/lib.rs` / `crates/store/src/proposal_records.rs` 增加 commit gate 事务 helper,用于同时检查 base version 与 pending proposal。
...
- 任何 lane 不得触碰 `crates/graph`、`crates/server/src/layout_routes.rs`、`crates/server/src/proposal_routes.rs`、`crates/agent/src/prompt_stack.rs`;除 backend-server lane 为 commit gate helper 明确列出的 `crates/store/src/lib.rs` / `crates/store/src/proposal_records.rs` 外,不得改动其他 `crates/store` 文件。
