...
| `SP153-T1` | store | `T0` | 新增 repair chain，以安全 table rebuild 扩展 continuation fix states，增加 attempts/outbox/user-cancel finalizer；用 conditional CAS + unique attempt 单赢家。 | 旧 rows 无损迁移；hold/interrupt 原子写 child interrupted + attempt cancelled + continuation completed；不 claim 下一 fix。 | `cargo test -p helixflow-store run_fix` |
...
| `SP153-T5` | recovery | `T1`–`T4` | fix policy 先于 GH154 stale classification；关闭时 quiesce 并释放 busy slot，用户 hold/interrupt 原子 user-cancel；开启时在正常 claim 下恢复；remote handle 只交 GH154 收敛。 | disabled manual run 可启动；preparing/ready child user-cancel 跨 reopen 永不恢复；reenable busy/current guard 与 crash matrix 通过。 | `cargo test -p helixflow-server run_agent_fix_recovery` |
