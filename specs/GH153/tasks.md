...
| `SP153-T1` | store | `T0` | 新增 repair chain，以安全 table rebuild 扩展 GH154 continuation fix states，增加 attempts/outbox API；用 conditional CAS + unique attempt 单赢家，不假设 lease。 | 所有 GH154 旧 state/row/linkage 无损迁移；不复制 handoff；并发 claim 单赢家；max=0/非法/N>1 exactly-once。 | `cargo test -p helixflow-store run_fix` |
...
| `SP153-T5` | recovery | `T1`–`T4` | fix policy 先于 GH154 stale classification；关闭时 quiesce 未派发状态并从 workspace busy 查询排除，开启时在正常 workspace claim 下恢复；remote handle 只交 GH154 收敛。 | disabled 后 manual run 可启动、旧 child 不执行；reenable busy 等待/current 变化拒绝；完整 recovery crash matrix 通过。 | `cargo test -p helixflow-server run_agent_fix_recovery` |
