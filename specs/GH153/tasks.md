...
| `SP153-T4` | run cost gate | `T3` | 从 target graph fresh compile/resolve/estimate，持久化 `child_preparing`，复用 GH154 stable estimate keys 幂等补齐；在 estimate/auto-start/manual confirm/restart/dispatch 每次重验 provider scope；完成后复用 confirmation/execute。 | 每个 estimate crash window reopen 可恢复；等待确认后换 credential/account 必须拒绝；unknown/over budget pending，within budget 自动执行；不重复 ledger。 | `cargo test -p helixflow-run run_agent_fix && cargo test -p helixflow-server run_agent_fix` |
...
- 修复 version 提交必须同时 CAS current version、nullable provider selection、
  ...
  dispatch 前还要重验，只做 current CAS 不满足本规格。
