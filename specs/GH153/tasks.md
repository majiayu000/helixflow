...
| `SP153-T1` | store | `T0` | 新增 repair chain、扩展 GH154 failure continuation、增加 `run_fix_attempts` 与 event outbox migration/API、状态机、operation replay 与 child linkage。 | 不复制 handoff；fix 计数独立；并发 claim 单赢家；max=0/非法/N>1 与重复 finalizer exactly-once；FK/cascade 正确。 | `cargo test -p helixflow-store run_fix` |
...
| `SP153-T4` | run cost gate | `T3` | 从 target graph fresh compile/resolve/estimate，持久化 `child_preparing`，复用 GH154 stable estimate keys；在 child create/estimate/auto-start/manual confirm/restart/dispatch 每次重验 current target、nullable selector 与 provider scope。 | rollback/deselect 或换 credential/account 全部拒绝旧 child；partial estimate 可恢复；cost gate/ledger 幂等。 | `cargo test -p helixflow-run run_agent_fix && cargo test -p helixflow-server run_agent_fix` |
| `SP153-T5` | recovery | `T1`–`T4` | fix policy 先于 GH154 stale classification；关闭时 quiesce 未派发状态，开启时恢复 continuation/attempt/outbox、Agent、version 与 partial estimate；remote handle 只交 GH154 收敛。 | disabled/reenabled matrix、provenance、Agent、target/version、每步 estimate、event crash 全覆盖；不重放 Agent/version/ledger。 | `cargo test -p helixflow-server run_agent_fix_recovery` |
...
- GH-153 规格可以先合并，但 implementation 必须在 GH-154 的 common finalizer、
