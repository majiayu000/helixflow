...
| `SP154-T3` | run | `T1`,`T2` | executor 在 submit 前写 dispatching，handle CAS active；让 sync dispatch、async resume、builtin、cache hit 通过 typed state matrix 共用幂等 step finalizer，实现 durable output reconstruction、剩余 DAG continuation 和 DB-based online interrupt。 | 五个 crash window 不重提；cache hit 可恢复；重复 resume 不复制 artifact/cost/event；非最终 step 先完成后仍可 interrupt，最终 step/interrupt/cancel 竞态只有一个 run terminal。 | `cargo test -p helixflow-run --locked recovery` |
