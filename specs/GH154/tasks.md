...
| --- | --- | --- | --- | --- | --- |
| `SP154-T0` | test/contracts | none | 固化 restart recovery 状态矩阵、三类 dispatch failure 和五个 crash-point fixtures；覆盖 queued/estimating/waiting/running、dispatching/active/result_ready/terminal、Atlas/fal/mock。 | 当前实现可稳定复现 handle 丢失；not-submitted/rejected/unknown 可区分；fixture 不触发真实付费调用且无 secret。 | `cargo test -p helixflow-run --locked recovery_baseline` |
| `SP154-T1` | store | `T0` | 增加带 dispatch owner/origin/scope/deadline、result_ready/spool/materialization retry 且 step 唯一的 provider tasks，以及 outputs/cost/lease/execution/terminalization/continuation/artifact journal API。 | live dispatch 不被 abandon；result_ready+actual cost 先于 artifact；materialization deadline 跨重启不延长；旧 DB 可升级且 CAS/FK/cascade 通过。 | `cargo test -p helixflow-store --locked run_recovery` |
...
- sync `dispatching→result_ready→completed`、async success
  `active→result_ready→completed`、async failed `active→completed`、builtin/cache 无 task
  row finalizer；
...
- succeeded 零输出 sink + active provider sibling 的 restart recovery；
