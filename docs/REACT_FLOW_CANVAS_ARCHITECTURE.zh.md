...
目标代表“编辑器可流畅操作”，不是承诺所有机器、所有节点模板都稳定 60 FPS。详细编辑态由 React Flow 管理；低缩放全图态使用单个 Canvas 位图作为 LOD，React Flow 仍负责视口和坐标系统，避免“适应全部”重新挂载 4000 个 DOM 节点。
...
DOM 裁剪已经达到目标，且边 LOD 消除了 6000 边带来的秒级额外阻塞。

### 2026-08-30 p95 门禁

真实 Chromium 运行 5 次冷启动和 10 秒滚轮/RAF 轨迹，本机结果如下：

| 指标 | 门槛 | 本机实测 |
|---|---:|---:|
| 冷启动 P95 | ≤ 2,500 ms | 1,745.8 ms |
| 帧间隔 P95 | ≤ 32 ms | 10.0 ms（1,200 样本） |
| 初始详细节点 DOM | < 100 | 通过 |
...
| 一次性迁移范围大 | 中 | 中 | 保持 `GraphCanvasProps` 领域契约稳定；已删除无入口引用的旧拖动、连线和缩放控制器 |
