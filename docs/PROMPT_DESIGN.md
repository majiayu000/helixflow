...
3. Workflow design is dynamic.
   ...
   The agent does not follow a hardcoded graph template. It reads the current graph and node/provider catalogs, then proposes graph operations that match the user's goal.

4. Catalogs are authoritative.

   The prompt may tell the agent how to reason, but valid node types, ports, params, and provider capabilities come from catalog files.

5. Runtime providers are open-ended.

   Atlas is not a special hardcoded provider. It is one runtime provider connector among many possible API connectors. The prompt should consume whatever runtime provider capabilities the backend exposes, without assuming that Atlas, ComfyUI, or any specific API must exist.
...
| `researchCommandContract` | `researchCommandContract` | When discovery is allowed. In Helixflow this mostly means graph/catalog/provider inspection, not broad shell research. |
| `runContextPrompt` | `runContextPrompt` | Current workspace, graph version, run/error context, pending proposal context, and selected provider state. |
| `browserUsePromptGuard` | `providerUsePromptGuard` | Provider/API boundary. Atlas or Comfy calls go through backend provider abstractions, not prompt-invented raw endpoints. |
...
| "运行", "提交到 Atlas", "生成结果" | valid graph | `RunRequest` | provider run request, not graph design |
...
- Provider/run mode: use backend provider capabilities only; do not invent shell/API calls.
...
- Create/modify workflow: `ctx/graph.json`, `ctx/node_defs/catalog.json`, optional `ctx/providers/catalog.json`.
- Debug workflow: graph, catalog, provider catalog, and `ctx/run_context.json`.
- Run request: graph plus provider executable status.
...
- Atlas, ComfyUI, or future providers are represented as provider capabilities.
- The agent may reference capability ids and provider-backed node types from catalogs.
- The agent must not print credentials, raw endpoints, signed URLs, or local secret paths.
- The agent must not simulate provider output.
- The agent must not claim that a real provider run succeeded unless the backend run record says so.
...
Use only node types, ports, params, and provider capabilities from the catalogs.
...
Validate that the graph has the required provider-backed executable nodes.
...
```json
{
  ...
  "summary": "Creates a text-to-image workflow using the available provider-backed image generation node.",
  "ops": [
    {
      "op": "add_node",
      "id": "prompt",
...
The prompt should not hardcode Atlas. It should consume a provider catalog emitted by the backend.
...
```text
Provider capabilities in ctx/providers/catalog.json describe what can be used. They do not expose credentials. Use them only by creating or connecting provider-backed graph nodes. Do not invent provider ids, node types, params, prices, or endpoints.
```

This lets the first Atlas integration be real without teaching the prompt raw Atlas API details.
...
- Include provider catalog when available.
...
3. Add provider catalog context.

   Generate `ctx/providers/catalog.json` from registered backend providers. For the first pass, Atlas capabilities can be the only real provider entries.
...
4. Add prompt tests.
   ...
   - provider prompt forbids mock providers and raw endpoints;
...
7. Atlas integration is represented as provider capabilities, not prompt hardcoding.
